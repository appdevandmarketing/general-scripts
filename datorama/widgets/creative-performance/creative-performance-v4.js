...
const CALCULATED_VALUES = {
  ...
  [FIELD_AD_THUMBNAIL]: (
    ...
    if (
      datoRows != null &&
      datoRows.length > 0 &&
      BID_STRATEGY_FOR_PAID_SEARCH.includes(
        datoRows[0][FIELD_CAMPAIGN_CATEGORY]
      )
    ) {
            ...
            doc.autoTable({
              ...
              didDrawCell: function (data) {
                ...
                if (data.column.index === 7 && data.cell.section === "body") {
                  ...
                  const startY =
                    links.length > 1
                      ? y + (cellHeight - totalLinksHeight) / 2 + fontSize
                      : y +
                        (cellHeight - totalLinksHeight) / 2 +
                        fontSize +
                        5.5;
