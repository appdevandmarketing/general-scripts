...
const RANDOM_AD_THUMBNAILS = [
  "https://cdn1.rainlocal.com/asset/creative/4955/840/bbf29e4d-cb9b-4.jpg",
  "https://cdn1.rainlocal.com/asset/creative/4955/840/6484d3b7-b559-4.jpg",
...
async function fetchAdLinks(datoFieldIndex, result) {
  ...
  try {
    ...
    Object.entries(adsLinkData).forEach(([adNumber, rows]) => {
      rows.forEach((row) => {
        const random =
          RANDOM_AD_THUMBNAILS[
            Math.floor(Math.random() * RANDOM_AD_THUMBNAILS.length)
          ];
        row.url = random;
