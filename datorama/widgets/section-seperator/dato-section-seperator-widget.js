...
const VIEW_CONFIGS = {
  "Google Analytics Traffic Details": {
    renderer: async (sectionName, advertiserData) => {
      ...
      return createBasicHtmlWithLink(
        sectionName,
        "fad fa-th",
        lookerStudioUrl,
        "Looker Studio"
  ...
  "Creative Performance": {
    renderer: async (sectionName, advertiserData) => {
      const creativeLink = getCreativeLink(advertiserData);
      return createBasicHtmlWithLink(
        sectionName,
        "fad fa-th",
        creativeLink,
...
async function getAdvertiserDetails(advertiserName, authToken) {
  try {
    const response = await fetch(
      "https://xconnect.rainlocal.com/open/v1/advertiser/search",
      {
        method: "POST",
        headers: {
...
function getLookerStudioLink(advertiserData) {
  if (
    advertiserData &&
    advertiserData.additionalMetadata &&
    advertiserData.additionalMetadata.links
  ) {
    const lookerLink = advertiserData.additionalMetadata.links.find(
      (link) => link.name === "Looker Studio"
    );
...
function getCreativeLink(advertiserData) {
  if (
    advertiserData &&
    advertiserData.additionalMetadata &&
    advertiserData.additionalMetadata.links
  ) {
    const creativeLink = advertiserData.additionalMetadata.links.find(
      (link) => link.name === "Creative Review Portal"
    );
    return creativeLink ? creativeLink.value : null;
  }
  return null;
}
...
async function renderView(sectionName) {
  ...
  if (config && config.renderer) {
    ...
    if (advertiserName && authToken) {
      ...
      console.error(
        "Could not extract advertiser name or auth token from data"
      );
    }
